...
4. **No Mandatory Test Coverage:** While testing is encouraged, there is no minimum code coverage requirement for this project.
...
8. **Record Task Summary:**
   - **Step 8.1: Draft Summary:** Create a detailed summary for the completed task. This should include the task name, a summary of changes, a list of all created/modified files, and the core "why" for the change.
   - **Step 8.2: Save Summary:** Store this summary locally (e.g., in a temporary file or internal state) to be included in the phase-end commit message.
...
2.  **Commit Phase Code Changes:**
    -   **Step 2.1: Stage Changes:** Stage all code changes related to the tasks in the completed phase.
    -   **Step 2.2: Compile Summaries:** Combine all task summaries gathered during the phase into a single, detailed commit message body.
    -   **Step 2.3: Perform Commit:** Commit the staged changes. The message should follow the format `<type>(<scope>): <phase_description>` with the compiled summaries in the body.
